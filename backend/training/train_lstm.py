...
def train_lstm():
    ...
    # Reshape to (batch, seq_len=1, features) — single-step sequence per sample
    ...
    train_loader = DataLoader(train_ds, batch_size=config.model.lstm_batch_size, shuffle=True)

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = LSTMClassifier().to(device)
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.Adam(model.parameters(), lr=config.model.lstm_learning_rate)
    ...
    for epoch in range(config.model.lstm_epochs):
        model.train()
        total_loss = 0
        ...
        if (epoch + 1) % 10 == 0:
            print(f"  Epoch {epoch + 1}/{config.model.lstm_epochs} — Loss: {total_loss / len(train_loader):.4f}, Acc: {acc:.4f}")
    ...
    print(f"\n✅ LSTM Best Accuracy: {best_acc:.4f}")
    ...
    print(f"💾 Model saved to {os.path.join(models_dir, 'lstm.pt')}")
